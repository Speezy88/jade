...
1. **Wrong tool for JADE's workflow.** NotebookLM excels at deep analysis of documents you *already have*. JADE's research need is the opposite — starting from a topic or project goal and finding the best sources. That's a source-discovery problem.
...
### Why Not Perplexity API (for now)

Perplexity API requires a $50 minimum credit deposit to get started. At JADE's actual research volume (a few queries per month for skills + occasional project research), spend would be roughly $3–8/month — meaning the deposit lasts 6–12 months. The cost per query is reasonable; the upfront barrier isn't worth it right now.

**Decision: Launch Phase 2.7 with Claude web search as the source-finding layer.** Claude already has web search capability built into the Anthropic API — no new credential, no new cost, already integrated into JADE's existing API spend. Research quality is slightly lower than Perplexity Deep Research for exhaustive multi-source synthesis, but more than sufficient for JADE's use case.

**Upgrade path:** When research volume grows or query depth becomes a limitation, swap in Perplexity by adding `PERPLEXITY_API_KEY` to `~/.config/jade/credentials` and flipping a config flag in `jade_research.py`. No architectural rewrite needed — the adapter pattern handles it.
...
jade_research.py
      │
      ├─→ Source Finding — Claude web search (Haiku)
      │       - Multi-query web search on the topic
      │       - Returns ranked sources with citations
      │       - No additional API cost — uses existing Anthropic spend
      │
      ├─→ Academic Layer (conditional) — Semantic Scholar API (free)
...
| Tool | Role | Automated? | Cost |
|------|------|-----------|------|
| Claude web search (Haiku) | Source discovery + web research | ✅ Yes | Existing API spend |
| Semantic Scholar API | Academic source finding | ✅ Yes (conditional) | Free |
| Claude (Haiku) | Synthesis + applied insights | ✅ Yes | Existing API spend |
| Notion Research Vault | Persistent storage of all findings | ✅ Yes | Free tier |
| NotebookLM | Deep-dive on specific document sets | ❌ Manual only | Free |
| Perplexity Deep Research | Upgrade path for deeper source finding | 🔜 Future | ~$0.40/query |
...
Phase 2.7 third — Research layer depends on both
  ├── Build jade_research.py with Claude web search as source finder
  ├── Add conditional Semantic Scholar for academic topics (free API, no key)
  ...
  ├── Add adapter interface for future Perplexity upgrade (config flag only)
...
- ISC-17: Academic source layer (Semantic Scholar) activates only for School/WTT/Reading area topics
- ISC-18: NotebookLM is NOT called at any point in the automated pipeline
- ISC-19: Perplexity adapter interface exists as a config flag — switching requires no code rewrite

### Phase 2.8
- ISC-20: New skill created via single command with all schema fields populated
- ISC-21: Initial research job produces full staged roadmap with Learn/Apply/Observe/Mentor per stage
- ISC-22: Stage advancement marks stage complete, advances current stage, triggers focused research
- ISC-23: Opportunities DB replaced (not appended) on each research cycle
- ISC-24: Practice Log entries created only for High priority skills during nightly check-in
- ISC-25: High priority skills cold for 14+ days flagged in morning briefing
- ISC-26: Monthly refresh runs only for High priority skills, skips Medium and Low
- ISC-27: New skill addable end-to-end (page + research + roadmap) in under 2 minutes
...
| Perplexity API vs Claude web search | Claude web search for launch — zero incremental cost, uses existing Anthropic spend. Perplexity added later via config flag when/if query depth becomes a limitation. $50 minimum deposit not worth it at current volume. |
| Academic source finding | Semantic Scholar free public API — no key required, sufficient for school/WTT/reading topics |
